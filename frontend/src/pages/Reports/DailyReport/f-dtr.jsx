...
const FDTR = () => {
        ...
        if (Array.isArray(allEmployees) && allEmployees.length > 0) {
          ...
          if (role === 'L2' || role === 'tl') {
            ...
            if (reportingEmployees.length > 0) {
              // Add TL themselves at the beginning of the list
              const tlEmployee = {
                employeeCode: employeeCode,
                firstName: firstName || username || 'Team Leader',
                lastName: '',
                isTL: true // Flag to identify TL in the list
              };
              setEmployeeOptions([tlEmployee, ...reportingEmployees]);
            } else {
              // If no reporting employees, show TL only
              const tlEmployee = {
                employeeCode: employeeCode,
                firstName: firstName || username || 'Team Leader',
                lastName: '',
      ...
      if (response && response.all_employees) {
        ...
        const activeTeamLeaders = (response.team_leaders || [])
          ...
          .map(tl => ({
            ...tl,
            // Remove emojis and clean the firstName
            firstName: tl.firstName ? tl.firstName.replace(/[\u{1F600}-\u{1F64F}]|[\u{1F300}-\u{1F5FF}]|[\u{1F680}-\u{1F6FF}]|[\u{1F1E0}-\u{1F1FF}]|[\u{2600}-\u{26FF}]|[\u{2700}-\u{27BF}]/gu, '').trim() : tl.firstName
          }));
  ...
  // F-DTR: Filters by CREATED_AT date (when candidate was first created)
  const fetchCandidates = async (fromDate = null, toDate = null, filterParams = null) => {
    ...
    try {
      ...
      // F-DTR specific: Use created_at date instead of updated_at
      params.append('date_field', 'created_at'); // Filter by creation date
      ...
      const transformedData = candidatesArray.map(candidate => {
        ...
        return {
          ...
          // Dates - F-DTR uses created_at for dateOfEntry
          dateOfEntry: candidate.created_at 
      ...
      console.log('✅ F-DTR: Transformed candidates data:', transformedData.length, 'records');
      if (transformedData.length > 0) {
        console.log('📋 F-DTR Sample candidate:', transformedData[0]);
        console.log('🔍 F-DTR dateOfEntry (should be created_at):', transformedData[0].dateOfEntry);
        console.log('🔍 F-DTR created_at:', transformedData[0].created_at);
        console.log('🔍 F-DTR updated_at:', transformedData[0].updated_at);
      }
        ...
        <div className="px-2 py-1">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h1 className="text-md font-bold text-gray-900">F-DTR Report</h1>
              </div>
            </div>
                ...
                >
                  ...
                  {employeeOptions.map((employee, index) => (
                    <option key={employee.employeeCode} value={employee.employeeCode}>
                      {employee.isTL 
                        ? `${employee.employeeCode} - ${employee.firstName} (TL)` 
