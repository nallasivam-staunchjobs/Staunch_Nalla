...
function Source() {
  ...
  useEffect(() => {
    const fetchSources = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/sources/`);
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(errorData?.name?.[0] || 'Something went wrong');
    }
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/sources/${sourceToDelete}/`, {
        method: 'DELETE',
      });

      if (!res.ok) throw new Error('Delete failed');

      setSources(sources.filter((item) => item.id !== sourceToDelete));
  ...
  //   );
  //   if (isDuplicate) {
  //     showToast('Another source with the same name already exists!', 'error');
  //     return;
  //   }
  ...
  const handleSaveEdit = async (id) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/sources/${id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          name: updatedName,
          status: sources.find((item) => item.id === id)?.status,
        }),
      });
      ...
      setSources(
        sources.map((item) => (item.id === id ? updated : item))
      );
  ...
  // Count active and deactive sources
  const activeCount = sources.filter((item) => item.status === 'Active').length;
  const deactiveCount = sources.filter(
    (item) => item.status === 'Deactive'
  ).length;

  const totalPages = Math.ceil(filteredSources.length / entriesPerPage);
  ...
  return (
    <div className="text-black">
      <div className="bg-white p-[15px] rounded-[10px] shadow-sm flex justify-between mb-4">
        <h1 className="text-lg font-bold">Source Management</h1>
            ...
            <table className="w-full text-sm sm:text-sm text-left border border-gray-200">
              <thead className="bg-white text-xs uppercase text-black border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-2 text-start">Source</th>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold">Add New Source</h2>
              <button
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold ms-1">
                {sources.find((item) => item.id === sourceToDelete)?.name ||
                  'this Source'}
              </span>
              ?
            </p>
      ...
      {statusChangeModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-4 w-full max-w-lg mx-auto">
            <h2 className="text-lg font-bold mb-4">Confirm Status Change</h2>
            <p className="mb-6">
              Are you sure you want to change the status of{' '}
              <span className="font-bold">
                "{statusChangeModal.sourceName}"
              </span>{' '}
              to{' '}
              <span
                className={`font-bold ${
      ...
      {editConfirmationModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <h2 className="text-lg font-bold mb-4">Edit Source</h2>
            <p className="mb-6">
              Do you want to edit the source{' '}
              <span className="font-bold">
