...
function Workmode() {
  // Initialize workmodes state from sampleMasterData, mapping 'Work Mode' to 'name'
  useEffect(() => {
    const fetchWorkmodes = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/workmodes/`);
        const data = await res.json();
        setWorkmodes(data);
      } catch (err) {
        console.error('Failed to fetch work modes:', err);
      }
    };
  ...
  // State for adding new Work Mode
  const [workmodes, setWorkmodes] = useState([]);
  const [newWorkMode, setNewWorkMode] = useState('');
  const [newWorkModeStatus, setNewWorkModeStatus] = useState('Active');
  const [showAddModal, setShowAddModal] = useState(false);

  // State for deleting Work Mode
  const [workModeToDelete, setWorkModeToDelete] = useState(null);
  const [showDeleteModal, setShowDeleteModal] = useState(false);

  // State for editing Work Mode
  ...
  // Handles adding a new Work Mode:

  const handleSubmit = async (e) => {
    e.preventDefault();
    const trimmed = newWorkMode.trim();
    if (!trimmed) return;

    const isDuplicate = workmodes.some(
      (item) => item.name.toLowerCase() === trimmed.toLowerCase()
    );
    if (isDuplicate) {
      showToast('Work Mode already exists!', 'error');
      return;
    }
    ...
    try {
      ...
      setWorkmodes([newEntry, ...workmodes]);
      setNewWorkMode('');
      setNewWorkModeStatus('Active');
      setShowAddModal(false);
      showToast(`${newEntry.name} added successfully!`);
    } catch (err) {
      console.error(err);
      showToast('Error adding work mode', 'error');
    }
  };

  
  // Initiates the delete process by setting the Work Mode to delete and showing the modal
  const handleDelete = (id) => {
    setWorkModeToDelete(id);
    setShowDeleteModal(true);
  };

  // Confirms and performs the deletion of a Work Mode: 

  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/workmodes/${workModeToDelete}/`, {
        method: 'DELETE',
      });
      if (!res.ok) throw new Error('Delete failed');
      const deletedItem = workmodes.find((item) => item.id === workModeToDelete);
      setWorkmodes(workmodes.filter((item) => item.id !== workModeToDelete));
      setShowDeleteModal(false);
      setWorkModeToDelete(null);
      showToast(`${deletedItem?.name || 'Work Mode'} deleted successfully!`, 'success');
    } catch (err) {
      console.error(err);
      showToast('Error deleting work mode', 'error');
    }
  };


  // Handles double click on a Work Mode name to show edit confirmation modal
  ...
  // Saves the edited Work Mode name:
  const handleSaveEdit = async (id) => {
    const updatedName = editValue.trim();
    if (!updatedName) return;

    // Check for duplicate Work Mode names (excluding the current one)
    const isDuplicate = workmodes.some(
      (item) =>
        item.id !== id && item.name.toLowerCase() === updatedName.toLowerCase()
    );
    if (isDuplicate) {
      showToast('Another Work Mode with the same name already exists!', 'error');
      return;
    }

    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/workmodes/${id}/`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          name: updatedName,
          status: workmodes.find((item) => item.id === id)?.status, // retain current status
        }),
      });
      ...
      setWorkmodes(
        workmodes.map((item) => (item.id === id ? updated : item))
      );
      ...
      console.error(err);
      showToast('Error updating work mode', 'error');
    }
  };
  ...
  const showStatusChangeConfirmation = (id) => {
    const workmode = workmodes.find((item) => item.id === id);
    setStatusChangeModal({
      show: true,
      id,
      newStatus: workmode.status === 'Active' ? 'Deactive' : 'Active',
      departmentName: workmode.name,
    });
  };
  ...
  // Filter workmodes based on search term
  const filteredWorkmodes = workmodes.filter((item) =>
    item.name?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Count active and deactive workmodes
  const activeCount = workmodes.filter((item) => item.status === 'Active').length;
  const deactiveCount = workmodes.filter(
    (item) => item.status === 'Deactive'
  ).length;

  // Calculate total pages for pagination
  const totalPages = Math.ceil(filteredWorkmodes.length / entriesPerPage);
  // Get workmodes for the current page
  const paginatedWorkmodes = filteredWorkmodes.slice(
  ...
  return (
    <div className="text-black">
      ...
      <div className="bg-white p-[15px] rounded-[10px] shadow-sm flex justify-between mb-4">
        <h1 className="text-lg font-bold">Work Mode Management</h1>
            ...
            <table className="w-full text-sm sm:text-sm text-left border border-gray-200">
              ...
              <thead className="bg-white text-xs uppercase text-black border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-2 text-start">Work Mode</th>
              ...
              <tbody>
                {paginatedWorkmodes.length === 0 ? (
                  ...
                  paginatedWorkmodes.map((item, index) => (
                    <tr
                      key={item.id}
                    ...
                    >
                      ...
                      {/* Work Mode name cell with double-click for editing */}
                      <td
              ...
              <div>
                <p className="text-xs text-gray-500">
                  ...
                  <span className="font-medium text-gray-700">
                    {Math.min(
                      currentPage * entriesPerPage,
                      filteredWorkmodes.length
                    )}
                  </span>{' '}
                  of{' '}
                  <span className="font-medium">{filteredWorkmodes.length}</span>{' '}
                  results
                </p>
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold">Add New Work Mode</h2>
              <button
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold ms-1">
                {workmodes.find((item) => item.id === workModeToDelete)?.name ||
                  'this Work Mode'}
              </span>
              ?
            </p>
      ...
      {editConfirmationModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <h2 className="text-lg font-bold mb-4">Edit Work Mode</h2>
            <p className="mb-6">
              Do you want to edit the Work Mode{' '}
              <span className="font-bold">
