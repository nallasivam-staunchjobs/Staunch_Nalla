...
function Communication() {
  // Initialize communications state from sampleMasterData, mapping 'Communication' to 'name'

  useEffect(() => {
      const fetchCommunications = async () => {
        try {
          const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/communications/`);
  ...
  // State for adding new Communication
  const [communications, setCommunications] = useState([]);
  
  const [newCommunication, setNewCommunication] = useState('');
  const [newCommunicationStatus, setNewCommunicationStatus] = useState('Active');
  const [showAddModal, setShowAddModal] = useState(false);

  // State for deleting Communication
  const [communicationToDelete, setCommunicationToDelete] = useState(null);
  const [showDeleteModal, setShowDeleteModal] = useState(false);

  // State for editing Communication
  ...
  // Handles adding a new Communication
  // const handleSubmit = (e) => {
  //   e.preventDefault();
  //   const trimmed = newCommunication.trim();
  //   if (!trimmed) return;

  //   // Check for duplicate Communication names (case-insensitive)
  //   const isDuplicate = communications.some(
  //     (item) => item.name.toLowerCase() === trimmed.toLowerCase()
  //   );
  //   if (isDuplicate) {
  //     showToast('Communication already exists!', 'error');
  //     return;
  //   }
  ...
  //     id: Date.now(),
  //     name: trimmed,
  //     status: newCommunicationStatus,
  //     createdAt: new Date().toISOString(),
  //   };

  //   setCommunications([newEntry, ...communications]);
  //   setNewCommunication('');
  //   setNewCommunicationStatus('Active');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      showToast('Error adding communication', 'error');
    }
  };


  // Initiates the delete process by setting the Communication to delete and showing the modal
  const handleDelete = (id) => {
    setCommunicationToDelete(id);
    setShowDeleteModal(true);
  };

  // Confirms and performs the deletion of a Communication
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/communications/${communicationToDelete}/`, {
        method: 'DELETE',
      });

      if (!res.ok) throw new Error('Delete failed');

      const deletedItem = communications.find((item) => item.id === communicationToDelete);
      setCommunications(communications.filter((item) => item.id !== communicationToDelete));
      setShowDeleteModal(false);
      setCommunicationToDelete(null);
      showToast(`${deletedItem?.name || 'Communication'} deleted successfully!`, 'success');
    } catch (err) {
      console.error(err);
      showToast('Error deleting communication', 'error');
    }
  };

  // Handles double click on a Communication name to show edit confirmation modal
  ...
  // Saves the edited Communication name
  // const handleSaveEdit = (id) => {
  //   const updatedName = editValue.trim();
  //   if (!updatedName) return;

  //   // Check for duplicate Communication names (excluding the current Communication being edited)
  //   const isDuplicate = communications.some(
  //     (item) =>
  //       item.id !== id && item.name.toLowerCase() === updatedName.toLowerCase()
  //   );
  //   if (isDuplicate) {
  //     showToast('Another Communication with the same name already exists!', 'error');
  //     return;
  //   }

  //   // Update the Communication name in the state
  //   setCommunications(
  ...
  const handleSaveEdit = async (id) => {
    ...
    // Check for duplicate Communication names (excluding the current one)
    const isDuplicate = communications.some(
      (item) =>
        item.id !== id && item.name.toLowerCase() === updatedName.toLowerCase()
    );
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/communications/${id}/`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          name: updatedName,
          status: communications.find((item) => item.id === id)?.status, // retain current status
        }),
      });
      ...
      setCommunications(
        communications.map((item) => (item.id === id ? updated : item))
      );
      ...
      console.error(err);
      showToast('Error updating communication', 'error');
    }
  };
  ...
  const showStatusChangeConfirmation = (id) => {
    ...
    setStatusChangeModal({
      show: true,
      id,
      newStatus: communication.status === 'Active' ? 'Deactive' : 'Active',
      departmentName: communication.name,
    });
  };
  ...
  // Count active and deactive communications
  const activeCount = communications.filter((item) => item.status === 'Active').length;
  const deactiveCount = communications.filter(
    (item) => item.status === 'Deactive'
  ).length;

  // Calculate total pages for pagination
  const totalPages = Math.ceil(filteredCommunications.length / entriesPerPage);
  // Get communications for the current page
  ...
  return (
    <div className="text-black">
      ...
      <div className="bg-white p-[15px] rounded-[10px] shadow-sm flex justify-between mb-4">
        <h1 className="text-lg font-bold">Communication Management</h1>
            ...
            <table className="w-full text-sm sm:text-sm text-left border border-gray-200">
              ...
              <thead className="bg-white text-xs uppercase text-black border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-2 text-start">Communication</th>
                ...
                {paginatedCommunications.length === 0 ? (
                  ...
                  paginatedCommunications.map((item, index) => (
                    ...
                    >
                      ...
                      {/* Communication name cell with double-click for editing */}
                      <td
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold">Add New Communication</h2>
              <button
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold ms-1">
                {communications.find((item) => item.id === communicationToDelete)?.name ||
                  'this Communication'}
              </span>
              ?
            </p>
      ...
      {editConfirmationModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <h2 className="text-lg font-bold mb-4">Edit Communication</h2>
            <p className="mb-6">
              Do you want to edit the Communication{' '}
              <span className="font-bold">
