...
function Industry() {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/industries/`)
      .then((res) => res.json())
      .then((data) => {
        setIndustries(data);
      })
  ...
  // State for adding new industry
  const [newIndustry, setNewIndustry] = useState('');
  const [newIndustryStatus, setNewIndustryStatus] = useState('Active');
  const [showAddModal, setShowAddModal] = useState(false);

  // State for deleting industry
  const [industryToDelete, setIndustryToDelete] = useState(null);
  const [showDeleteModal, setShowDeleteModal] = useState(false);

  // State for editing industry
  ...
  const [statusChangeModal, setStatusChangeModal] = useState({
    ...
    id: null,
    newStatus: '',
    industryName: '', // Renamed from sourceName
  });
  ...
  // Check for duplicate industry names (case-insensitive)
  const isDuplicate = industries.some(
      (item) => item.name.toLowerCase() === trimmed.toLowerCase()
    );
    if (isDuplicate) {
      showToast('Industry already exists!', 'error');
      return;
    }

    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/industries/`, {
        method: 'POST',
        headers: {
      ...
      // Add newly created industry to the list
      setIndustries([data, ...industries]);

      setNewIndustry('');
      setNewIndustryStatus('Active');
      setShowAddModal(false);
      showToast(`${data.name} added successfully!`);
    } catch (error) {
      console.error(error);
      showToast('Something went wrong while creating industry.', 'error');
    }
  };


  // Initiates the delete process by setting the industry to delete and showing the modal
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/industries/${industryToDelete}/`, {
        method: 'DELETE',
      });
      ...
      setIndustries(industries.filter((item) => item.id !== industryToDelete));
  ...
  // Handles double click on an industry name to show edit confirmation modal
  ...
  const startEditing = () => {
    setEditingId(editConfirmationModal.id); // Set the ID of the industry being edited
  ...
  // Saves the edited industry name
  const handleSaveEdit = async (id) => {
    ...
    if (isDuplicate) {
      showToast('Another industry with the same name already exists!', 'error');
      return;
    }

    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/industries/${id}/`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
        body: JSON.stringify({
          name: updatedName,
          status: industries.find((item) => item.id === id)?.status, // retain current status
        }),
      });
      ...
      // Update local state
      setIndustries(
        industries.map((item) => (item.id === id ? updated : item))
      );
      ...
      showToast('Error updating industry', 'error');
      console.error(err);
    }
  };
  ...
  const showStatusChangeConfirmation = (id) => {
    ...
    setStatusChangeModal({
      show: true,
      id,
      newStatus: industry.status === 'Active' ? 'Deactive' : 'Active', // Toggle status
      industryName: industry.name, // Pass industry name for display
    });
  };
...
const confirmStatusChange = async () => {
  ...
  try {
    const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/industries/${id}/`, {
      ...
      body: JSON.stringify({
        name: industry.name,     // Keep name the same
    ...
    showToast(`${updated.name} status changed to ${updated.status}`, 'success');
  } catch (error) {
    console.error(error);
    showToast('Failed to update industry status', 'error');
  }
};
  ...
  // Count active and deactive industries
  const activeCount = industries.filter((item) => item.status === 'Active').length;
  const deactiveCount = industries.filter(
    (item) => item.status === 'Deactive'
  ).length;

  // Calculate total pages for pagination
  const totalPages = Math.ceil(filteredIndustries.length / entriesPerPage);
  // Get industries for the current page
  ...
  return (
    <div className="text-black">
      ...
      <div className="bg-white p-[15px] rounded-[10px] shadow-sm flex justify-between mb-4">
        <h1 className="text-lg font-bold">Industry Management</h1>
            ...
            <table className="w-full text-sm sm:text-sm text-left border border-gray-200">
              ...
              <thead className="bg-white text-xs uppercase text-black border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-2 text-start">Industry</th>
                ...
                {paginatedIndustries.length === 0 ? (
                  ...
                  paginatedIndustries.map((item, index) => (
                    ...
                    >
                      ...
                      {/* Industry name cell with double-click for editing */}
                      <td 
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <div>
                <p className="text-xs text-gray-500">
                  ...
                  of{' '}
                  <span className="font-medium">{filteredIndustries.length}</span>{' '}
                  results
                </p>
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold">Add New Industry</h2>
              <button
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold ms-1">
                {industries.find((item) => item.id === industryToDelete)?.name ||
                  'this Industry'}
              </span>
              ?
            </p>
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold">
                "{statusChangeModal.industryName}"
              </span>{' '}
              to{' '}
              <span
                className={`font-bold ${
      ...
      {editConfirmationModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <h2 className="text-lg font-bold mb-4">Edit Industry</h2>
            <p className="mb-6">
              Do you want to edit the industry{' '}
              <span className="font-bold">
