...
function Position() {
  // Initialize positions state from sampleMasterData, mapping 'Position' to 'name'
  useEffect(() => {
    const fetchPositions = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/positions/`);
  ...
  const [newPositionStatus, setNewPositionStatus] = useState('Active');
  const [showAddModal, setShowAddModal] = useState(false);

  // State for deleting Position
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setPositions([newEntry, ...positions]);
      setNewPosition('');
      setNewPositionStatus('Active');
  ...
  // Initiates the delete process by setting the Position to delete and showing the modal
  const handleDelete = (id) => {
    setPositionToDelete(id);
    setShowDeleteModal(true);
  };

  // Confirms and performs the deletion of a Position
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/positions/${positionToDelete}/`, {
        method: 'DELETE',
      });

      if (!res.ok) throw new Error('Delete failed');

      const deletedItem = positions.find((item) => item.id === positionToDelete);
      setPositions(positions.filter((item) => item.id !== positionToDelete));
      setShowDeleteModal(false);
      setPositionToDelete(null);
      showToast(`${deletedItem?.name || 'Position'} deleted successfully!`, 'success');
    } catch (err) {
      console.error(err);
  ...
  // Handles double click on a Position name to show edit confirmation modal
  ...
  // Saves the edited Position name
  const handleSaveEdit = async (id) => {
    const updatedName = editValue.trim();
    if (!updatedName) return;

    // Check for duplicate Position names (excluding the current one)
    const isDuplicate = positions.some(
      (item) => item.id !== id && item.name.toLowerCase() === updatedName.toLowerCase()
    );
    if (isDuplicate) {
      showToast('Another Position with the same name already exists!', 'error');
      return;
    }

    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/positions/${id}/`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          name: updatedName,
          status: positions.find((item) => item.id === id)?.status, // retain current status
        }),
      });
      ...
      setPositions(
        positions.map((item) => (item.id === id ? updated : item))
      );
  ...
  const showStatusChangeConfirmation = (id) => {
    ...
    setStatusChangeModal({
      show: true,
      id,
      newStatus: position.status === 'Active' ? 'Deactive' : 'Active',
  ...
  // Count active and deactive positions
  const activeCount = positions.filter((item) => item.status === 'Active').length;
  const deactiveCount = positions.filter(
    (item) => item.status === 'Deactive'
  ).length;

  // Calculate total pages for pagination
  const totalPages = Math.ceil(filteredPositions.length / entriesPerPage);
  // Get positions for the current page
  ...
  return (
    <div className="text-black">
      ...
      <div className="bg-white p-[15px] rounded-[10px] shadow-sm flex justify-between mb-4">
        <h1 className="text-lg font-bold">Position Management</h1>
            ...
            <table className="w-full text-sm sm:text-sm text-left border border-gray-200">
              ...
              <thead className="bg-white text-xs uppercase text-black border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-2 text-start">Position</th>
                ...
                {paginatedPositions.length === 0 ? (
                  ...
                  paginatedPositions.map((item, index) => (
                    ...
                    >
                      ...
                      {/* Position name cell with double-click for editing */}
                      <td
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold ms-1">
                {positions.find((item) => item.id === positionToDelete)?.name ||
                  'this Position'}
              </span>
              ?
            </p>
      ...
      {editConfirmationModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              Do you want to edit the Position{' '}
              <span className="font-bold">
