...
function Gender() {
  // Initialize genders state from sampleMasterData, mapping 'Gender' to 'name'
  // const [genders, setGenders] = useState(() =>
  //   sampleMasterData.map((item, index) => ({
  //     id: Date.now() + index, // Unique ID for each Gender
  //     name: item['Gender'], // 'Gender' from sample data becomes 'name' for Gender
  ...
  // );
    ...
    useEffect(() => {
      const fetchGenders = async () => {
        try {
          const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/genders/`);
          if (!res.ok) {
  ...
  // State for adding new Gender
  const [genders, setGenders] = useState([]);
  const [newGender, setNewGender] = useState('');
  const [newGenderStatus, setNewGenderStatus] = useState('Active');
  const [showAddModal, setShowAddModal] = useState(false);

  // State for deleting Gender
  const [genderToDelete, setGenderToDelete] = useState(null);
  const [showDeleteModal, setShowDeleteModal] = useState(false);

  // State for editing Gender
  ...
  // Handles adding a new Gender
  // const handleSubmit = (e) => {
  //   e.preventDefault();
  //   const trimmed = newGender.trim();
  //   if (!trimmed) return;

  //   // Check for duplicate Gender names (case-insensitive)
  //   const isDuplicate = genders.some(
  //     (item) => item.name.toLowerCase() === trimmed.toLowerCase()
  //   );
  //   if (isDuplicate) {
  //     showToast('Gender already exists!', 'error');
  //     return;
  //   }
  ...
  //     id: Date.now(),
  //     name: trimmed,
  //     status: newGenderStatus,
  //     createdAt: new Date().toISOString(),
  //   };

  //   setGenders([newEntry, ...genders]);
  //   setNewGender('');
  //   setNewGenderStatus('Active');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error(err);
      showToast('Error adding gender', 'error');
    }
  };


  // Initiates the delete process by setting the Gender to delete and showing the modal
  const handleDelete = (id) => {
    setGenderToDelete(id);
    setShowDeleteModal(true);
  };

  // Confirms and performs the deletion of a Gender
  // const confirmDelete = () => {
  //   const deletedItem = genders.find((item) => item.id === genderToDelete);
  //   setGenders(genders.filter((item) => item.id !== genderToDelete));
  //   setShowDeleteModal(false);
  //   setGenderToDelete(null);
  //   showToast(
  //     `${deletedItem?.name || 'Gender'} deleted successfully!`,
  //     'error'
  //   );
  // };
  ...
  const confirmDelete = async () => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/genders/${genderToDelete}/`, {
        method: 'DELETE',
      });
      ...
      setShowDeleteModal(false);
      setGenderToDelete(null);
      showToast(`${deletedItem?.name || 'Gender'} deleted successfully!`, 'success');
    } catch (err) {
      console.error(err);
      showToast('Error deleting gender', 'error');
    }
  };


  // Handles double click on a Gender name to show edit confirmation modal
  ...
  // Saves the edited Gender name
  // const handleSaveEdit = (id) => {
  //   const updatedName = editValue.trim();
  //   if (!updatedName) return;

  //   // Check for duplicate Gender names (excluding the current Gender being edited)
  //   const isDuplicate = genders.some(
  //     (item) =>
  //       item.id !== id && item.name.toLowerCase() === updatedName.toLowerCase()
  //   );
  //   if (isDuplicate) {
  //     showToast('Another Gender with the same name already exists!', 'error');
  //     return;
  //   }

  //   // Update the Gender name in the state
  //   setGenders(
  //     genders.map((item) =>
  ...
  const handleSaveEdit = async (id) => {
    ...
    // Check for duplicate Gender names (excluding the current one)
    const isDuplicate = genders.some(
      (item) =>
        item.id !== id && item.name.toLowerCase() === updatedName.toLowerCase()
    );
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/masters/genders/${id}/`, {
        method: 'PUT',
        headers: {
        ...
        body: JSON.stringify({
          name: updatedName,
          status: genders.find((item) => item.id === id)?.status, // retain current status
        }),
      });
      ...
      setGenders(
        genders.map((item) => (item.id === id ? updated : item))
      );
  ...
  const showStatusChangeConfirmation = (id) => {
    ...
    setStatusChangeModal({
      show: true,
      id,
      newStatus: gender.status === 'Active' ? 'Deactive' : 'Active',
      departmentName: gender.name,
    });
  };
  ...
  // Count active and deactive genders
  const activeCount = genders.filter((item) => item.status === 'Active').length;
  const deactiveCount = genders.filter(
    (item) => item.status === 'Deactive'
  ).length;

  // Calculate total pages for pagination
  const totalPages = Math.ceil(filteredGenders.length / entriesPerPage);
  // Get genders for the current page
  ...
  return (
    <div className="text-black">
      ...
      <div className="bg-white p-[15px] rounded-[10px] shadow-sm flex justify-between mb-4">
        <h1 className="text-lg font-bold">Gender Management</h1>
            ...
            <table className="w-full text-sm sm:text-sm text-left border border-gray-200">
              ...
              <thead className="bg-white text-xs uppercase text-black border-b border-gray-200 sticky top-0 z-10">
                <tr>
                  ...
                  <th className="px-2 sm:px-4 py-2 text-start">Gender</th>
              ...
              <tbody>
                {paginatedGenders.length === 0 ? (
                  ...
                  paginatedGenders.map((item, index) => (
                    <tr
                      key={item.id}
                    ...
                    >
                      ...
                      {/* Gender name cell with double-click for editing */}
                      <td
            ...
            <div className="hidden sm:flex sm:flex-1 sm:items-center sm:justify-between">
              <div>
                <p className="text-xs text-gray-500">
                  ...
                  of{' '}
                  <span className="font-medium">{filteredGenders.length}</span>{' '}
                  results
                </p>
              </div>
              <div>
        ...
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold">Add New Gender</h2>
              <button
          ...
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            ...
            <p className="mb-6">
              ...
              <span className="font-bold ms-1">
                {genders.find((item) => item.id === genderToDelete)?.name ||
                  'this Gender'}
              </span>
              ?
            </p>
      ...
      {editConfirmationModal.show && (
        <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-2 sm:p-4">
          <div className="bg-white text-black rounded-lg shadow-lg p-4 sm:p-6 w-full max-w-md mx-auto">
            <h2 className="text-lg font-bold mb-4">Edit Gender</h2>
            <p className="mb-6">
              Do you want to edit the Gender{' '}
              <span className="font-bold">
