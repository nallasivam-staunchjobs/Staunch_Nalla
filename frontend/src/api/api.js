...
export const candidates = {
  ...
  // Fetch flattened ClientJob rows matching filters (job-level drilldown)
  getClientJobRows: async (filters = {}) => {
    try {
      const params = new URLSearchParams();
      Object.keys(filters || {}).forEach((key) => {
      ...
      const url = `/candidates/job-rows/?${params.toString()}`;
      const response = await API.get(url);
      return response.data; // { count, results }
    } catch (error) {
      console.error('Error fetching client job rows:', error.response?.data || error.message);
      throw error;
    }
  },
