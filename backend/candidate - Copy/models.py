...
class ClientJob(AuditFields):
    ...
    def add_feedback(self, feedback_text, remarks="", nfd_date=None, ejd_date=None, ifd_date=None, profile_submission=None, profile_submission_date=None, entry_by="", entry_time=None, entry_id=None, call_status="", update_date_fields=True):
        ...
        print(f"DEBUG MODEL: Current feedback before: '{self.feedback}'")
        
        # Handle update vs create logic
        if entry_id is not None:
            # Update existing entry
            if self.feedback:
                entries = self.feedback.split(';;;;;;')
                if 0 <= entry_id < len(entries):
                    entries[entry_id] = feedback_entry
                    self.feedback = ';;;;;;'.join(entries)
        ...
        try:
            self.feedback = clean_text(self.feedback) if self.feedback else ""
            print(f"DEBUG MODEL: New feedback after cleaning: '{self.feedback}'")
        except Exception as e:
            print(f"ERROR MODEL: Failed to clean final feedback: {str(e)}")
            # Fallback to ASCII-safe version
            try:
                self.feedback = str(self.feedback).encode('ascii', errors='ignore').decode('ascii') if self.feedback else ""
            except:
                self.feedback = "Feedback encoding error"
    ...
    def get_feedback_entries(self):
        ...
        try:
            ...
            if not isinstance(self.feedback, str):
                print(f"WARNING: feedback is not a string for ClientJob {self.id}: {type(self.feedback)}")
                return []
            ...
            return [{
                'feedback': str(self.feedback) if self.feedback else 'Error parsing feedback',
                'nfd_date': '',
                'nfd_status': '',
                'ejd_date': '',
                'ifd_date': '',
            ...
            try:
                ...
                if ('Feedback' in entry or entry.startswith(('Feedback', ':')) or 
                    ...
                    if len(parts) >= 3:
                        ...
                        parsed['entry_time'] = parts[2].strip()
                    elif len(parts) == 2:
                        parsed['feedback'] = parts[0].strip()
                        parsed['executive_name'] = parts[1].strip()
                    else:
    ...
    def check_and_update_expired_nfd(self):
        ...
        # 2. ENHANCED: Update ALL expired NFDs in feedback entries
        if self.feedback:
    ...
    def _update_all_expired_feedback_nfds(self, current_datetime):
        ...
        feedback_entries = [entry.strip() for entry in self.feedback.split(';') if entry.strip()]
        ...
        # Save updated feedback if any changes were made
        if feedback_updated:
            self.feedback = ';'.join(feedback_entries)
    ...
    def _update_expired_nfd_fast(self, current_datetime):
        ...
        try:
            ...
            if self.feedback and 'Auto updated to open profile due to expired NFD' in self.feedback:
                return False
...
class CandidateRevenue(AuditFields):
    ...
    accountable_ctc = models.IntegerField(default=None, null=True, blank=True)
    offer_ctc = models.IntegerField(default=None, null=True, blank=True)
...
class CandidateStatusHistory(models.Model):
    ...
    def create_status_entry(cls, candidate_id, remarks, change_date, created_by, extra_notes=None,
                        client_job_id=None, vendor_id=None, client_name=None, profile_submission=None):
        """
        ...
        """
        try:
            # Prevent duplicate 'Interested' entries for the same date
            if remarks and remarks.strip().lower() == "interested":
                ...
                if exists:
                    print(f"⚠ Skipped duplicate 'Interested' entry for candidate {candidate_id} on {change_date}")
                    return None
            ...
            if existing_entry:
                print(f"ℹStatus entry already exists for candidate {candidate_id}: {remarks}")
