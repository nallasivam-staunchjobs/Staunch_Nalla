...
class ClientJobSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        """Override to sanitize feedback field before serialization and convert attend to 0/1"""
        representation = super().to_representation(instance)
        
        try:
            if 'feedback' not in representation:
                representation['feedback'] = getattr(instance.candidate, 'feedback', None)
        except Exception:
            representation['feedback'] = None
        
        # Convert attend boolean to 0 or 1
        if 'attend' in representation:
            representation['attend'] = 1 if representation['attend'] else 0
...
class CandidateRevenueSerializer(serializers.ModelSerializer):
    ...
    def get_branch(self, obj):
        """Get the branch from the employee's record in empreg_employee table"""
        from empreg.models import Employee
        
        try:
            # Get the employee by matching the executive_name with employeeCode
            employee = Employee.objects.filter(
                employeeCode=obj.candidate.executive_name
            ).first()
            ...
            print(f"Error getting employee branch: {e}")
            
        # Fallback to client jobs if employee not found or branch not set
        if hasattr(obj.candidate, 'client_jobs') and obj.candidate.client_jobs.exists():
            latest_job = obj.candidate.client_jobs.order_by('-id').first()
            return latest_job.branch if hasattr(latest_job, 'branch') else None
            
        return None
    
    def get_profile_status(self, obj):
        """Get the profile status from candidate's client jobs"""
        if hasattr(obj.candidate, 'client_jobs') and obj.candidate.client_jobs.exists():
            # Get the most recent client job
            latest_job = obj.candidate.client_jobs.order_by('-id').first()
            return latest_job.profilestatus  # Changed from profile_status to profilestatus
        return None
    
    def get_client_name(self, obj):
        """Get the most recent client name from candidate's client jobs"""
        if hasattr(obj.candidate, 'client_jobs') and obj.candidate.client_jobs.exists():
            # Get the most recent client job
            latest_job = obj.candidate.client_jobs.order_by('-id').first()
            return latest_job.client_name
        return None
    
    def get_display_executive_name(self, obj):
        """Return formatted executive name (FirstName LastName) from employee code"""
        if not hasattr(obj.candidate, 'executive_name') or not obj.candidate.executive_name:
            return ""
        
        name = obj.candidate.executive_name
        try:
            from empreg.models import Employee
            from django.db.models import Q
            ...
            # Try to get from cache first
            cache_key = f"executive_display_name_{name}"
            ...
            if employee:
                ...
                if not display_name:
                    display_name = employee.employeeCode
            else:
                display_name = name
                
            # Cache the result for future use (1 hour)
            cache.set(cache_key, display_name, timeout=3600)
            return display_name
            
        except Exception as e:
            logger.error(f"Error getting executive display name: {str(e)}")
            return name

    def get_location(self, obj):
        """Return combined city and state with comma separator if both exist"""
        city = obj.candidate.city or ""
        state = obj.candidate.state or ""
        if city and state:
            return f"{city}, {state}"
    ...
    def to_representation(self, instance):
        ...
        try:
            for field in ('created_at', 'updated_at'):
                ...
                if isinstance(value, str):
                    try:
                        ...
                        data[field] = parsed_dt.strftime('%Y-%m-%d')
            ...
            import logging
            logger = logging.getLogger(__name__)
            logger.error(f"Error formatting date in CandidateRevenueSerializer: {str(e)}")
            
        return data

    def update(self, instance, validated_data):
        """Preserve existing percentage/amount when both are explicitly sent as null.

        This is mainly for Abscond/draft updates: frontend sends percentage=null and
        amount=null to indicate "no change" rather than "clear both".
        """
        try:
            if (
                'percentage' in validated_data and
                'amount' in validated_data and
                validated_data.get('percentage') is None and
                validated_data.get('amount') is None
            ):
                # Remove both keys so existing DB values are kept
                validated_data.pop('percentage', None)
                validated_data.pop('amount', None)
        except Exception:
            # In case of any unexpected issue, fall back to normal update
            pass
...
class CandidateRevenueFeedbackSerializer(serializers.ModelSerializer):
    ...
    def get_profile_status(self, obj):
        """Get the profile status from candidate's client jobs"""
        candidate_revenue = getattr(obj, 'candidate_revenue', None)
        candidate = getattr(candidate_revenue, 'candidate', None)

        if candidate and hasattr(candidate, 'client_jobs') and candidate.client_jobs.exists():
            # Get the most recent client job
            latest_job = candidate.client_jobs.order_by('-id').first()
            return latest_job.profilestatus  # Changed from profile_status to profilestatus
        return None
...
class ProfileInSerializer(serializers.ModelSerializer):
    ...
    feedback = serializers.CharField(source='candidate.feedback', read_only=True)
